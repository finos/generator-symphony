...
class CircleOfTrustHandler:
    ...
    @web.middleware
    async def add_cors_headers(self, request, handler):
        response = await handler(request)
        # Lines below need to be updated accordingly
    ...
    @web.middleware
    async def wrap_error(self, request, handler):
        try:
            return await handler(request)
        except AuthInitializationError as exc:
            logger.warning("AuthInitializationError occurred for request url: %s", request.url, exc_info=exc)
            raise web.HTTPUnauthorized(text=exc.message)
        except ApiException as exc:
            logger.warning("ApiException occurred for request url: %s", request.url, exc_info=exc)
            return web.Response(status=exc.status, reason=exc.reason, text=f"Error when calling the pod: {str(exc)}")
